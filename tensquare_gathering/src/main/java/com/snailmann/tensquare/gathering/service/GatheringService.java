package com.snailmann.tensquare.gathering.service;

import com.snailmann.tensquare.common.util.IdWorker;
import com.snailmann.tensquare.gathering.dao.GatheringDao;
...
public class GatheringService {
	 ...
	 * 根据ID查询实体
     * 这里使用了Spring Cache
     * value是全局唯一标识，相当于redis的一个Key
     * key是全局唯一标识中的某个Key标识，相当于局部唯一标识，相当于redis中双重嵌套
     * 虽然value没有什么卵用，但是不写会报错...key
	 * @param id
	 * @return
     *
     *
     * 这么写的作用是，以后要获取返回值Gathering对象时，会优先去gathering区域拿key为id的对象；而不是去数据库中
     * #id的作用就是获取参数id的值，这里的意思也是将id作为key值，Gathering对象作为value存入Spring缓存的gathering区域
	 */
